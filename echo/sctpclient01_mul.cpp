...
int main(int argc, char* argv[])
{
    ...
    while(getline(std::cin, line)) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            oss << line << ".msg " << i;
            std::string send_line = oss.str();
            int send_sz = unpd::sctp_sendmsg(sock_fd, send_line.c_str(), send_line.length(),
                                             (struct sockaddr*)&servaddr, sizeof(servaddr),
                                             0, 0, 
                                             i, // the stream number
                                             0, 0);
            if (send_sz == -1){
                std::cerr << "sctp_sendmsg err:" << unpd::getErrorString() << std::endl;
                return -1;
            }
        }
